...
class KolorsPipeline_DoE(DiffusionPipeline, StableDiffusionMixin, StableDiffusionXLLoraLoaderMixin, IPAdapterMixin, PAGMixin):
    r"""
    ...
    def __init__(
        ...
        self.mask_processor  = VaeImageProcessor(vae_scale_factor=self.vae_scale_factor,  
                                                 do_resize=False, do_normalize=False, do_binarize=False, do_convert_grayscale=True)
        self.control_image_processor = VaeImageProcessor(
            vae_scale_factor=self.vae_scale_factor, do_convert_rgb=True, do_normalize=False)

        self.default_sample_size = self.unet.config.sample_size if hasattr(self, "unet") and self.unet is not None else 128
    ...
    def get_timesteps(self, num_inference_steps, strength, device, denoising_start=None):
        if denoising_start is None:
            init_timestep = min(int(0.5 + num_inference_steps * strength), num_inference_steps)
    ...
    def __call__(
        ...
        nul_pooled_prompt_embeds: Optional[torch.Tensor] = None,
        do_bias_CFG: bool = False,
        ...
        (
            nul_prompt_embeds,
            nul_pooled_prompt_embeds
        ) = self.encode_prompt(
            None,
            num_images_per_prompt=num_images_per_prompt,
            device=device,
            prompt_embeds=nul_prompt_embeds,
            pooled_prompt_embeds=nul_pooled_prompt_embeds,
            max_sequence_length=max_sequence_length,
        )
        ...
        # if image is not None or control_image is not None:
            # self.vae.to('cuda')
            # if self.controlnet:
        ...
        if image is not None:
            ...
            if strength < 1.0:
                ...
                ts = timesteps[:1].repeat(num_images_per_prompt)
        ...
        if control_image is not None:
            ...
            control_image = control_image.to('cuda').to(latents.dtype)

            mult = 1
            if do_bias_CFG or pag_scale > 0.0:
                mult += 1
            if self.do_classifier_free_guidance:
                mult += 1
            control_image = torch.cat([control_image] * mult)
        ...
        if self.do_perturbed_attention_guidance:
            ...
            add_text_embeds = torch.cat([nul_pooled_prompt_embeds, negative_pooled_prompt_embeds, add_text_embeds], dim=0)
            add_time_ids = torch.cat([add_time_ids, negative_add_time_ids, add_time_ids], dim=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                thisStep = i / (num_timesteps - 1)

                if doDiffDiff and thisStep <= mask_cutoff:
                    tmask = (mask >= thisStep)
                ...
                if self.controlnet and thisStep >= control_start and thisStep <= control_end:
                    self.unet.to('cpu')
                    ...
                    down_block_res_samples, mid_block_res_sample = self.controlnet(
                        ...
                        conditioning_scale=controlnet_conditioning_scale,   #or set to 0.0 when outside start|end
                        guess_mode=False,
                ...
                if self.do_perturbed_attention_guidance:
                    ...
                    noise_pred_nul, noise_pred_uncond, noise_pred_text = noise_pred.chunk(3)
                    noise_pred = noise_pred_nul + guidance_scale * (noise_pred_text - noise_pred_uncond)
